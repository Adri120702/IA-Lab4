...
class DefensiveReflexAgent(OffensiveQLearning):
    ...
    def getFeatures(self, game_state, action):
        ...
        if len(pacman) > 0:  
            distTopacman = self.closest_pacman(my_pos, pacman , walls)
            features['distance_to_pacman'] = float(distTopacman) / (walls.width * walls.height)

        return features

    def get_weights(self, game_state, action):
        return {'num_invaders': -1000, 'on_defense': 100,  'stop': -100, 'reverse': -2, 'distance_to_pacman': -100}
